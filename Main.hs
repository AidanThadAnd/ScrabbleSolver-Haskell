...
-- Import necessary types and functions from other modules and libraries
import Solver (solveScrabble, Move(..), Direction(..)) -- Core solver logic and types
import Board (Square(..), loadBoard, printBoard, Bonus(..), letter, bonus, validateBoard) -- Board representation, loading, printing, validation
import System.Environment (getArgs)      -- For reading command-line arguments
import System.IO                         -- Standard Input/Output
import qualified Data.Array as A         -- Use qualified Array import

-- | Applies a found move (represented by its placement list) to an existing board state.
--   This function assumes the move placement is valid according to game rules
--   When placing a tile, any bonus on that square is considered consumed (set to None).
...
applyMove board placement = board A.// updates -- Uses the Array update operator (//)
    where
